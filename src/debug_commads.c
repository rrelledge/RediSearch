...
DEBUG_COMMAND(NumericIndexSummary) {
  ...
  GET_SEARCH_CTX(argv[0])
  size_t len;
  const char *fname = RedisModule_StringPtrLen(argv[1], &len);
  const FieldSpec *fs = IndexSpec_GetField(sctx->spec, fname, len);
  if (!fs) {
    RedisModule_ReplyWithError(sctx->redisCtx, "Could not find given field in index spec");
    goto end;
  }
  NumericRangeTree *rt = OpenNumericIndex(sctx, RedisModule_StringPtrLen(argv[1], NULL));
  if (!rt) {
...
DEBUG_COMMAND(DumpNumericIndex) {
  ...
  NumericRangeTree *rt = OpenNumericIndex(sctx, fname);
  if (!rt) {
    RedisModule_ReplyWithError(sctx->redisCtx, "can not open numeric field");
    goto end;
  }
...
DEBUG_COMMAND(DumpTagIndex) {
  ...
  GET_SEARCH_CTX(argv[0])
  TagIndex *tagIndex = TagIndex_Open(sctx, RedisModule_StringPtrLen(argv[1], NULL), false);
  if (!tagIndex) {
...
DEBUG_COMMAND(InfoTagIndex) {
  ...
  const TagIndex *idx = TagIndex_Open(sctx, RedisModule_StringPtrLen(argv[1], NULL), false);
  if (!idx) {
    RedisModule_ReplyWithError(sctx->redisCtx, "can not open tag field");
    goto end;
  }
