...
static TagIndex *openTagKeyDict(RedisSearchCtx *ctx, const char *key, int openWrite) {
  ...
  dictAdd(ctx->spec->keysDict, (void *)key, kdv);
  return kdv->p;
}

/* Open the tag index in redis */
TagIndex *TagIndex_Open(RedisSearchCtx *sctx, const char *formattedKey, int openWrite) {
